...
bool abb_iter_in_recur(nodo_abb_t* nodo,bool visitar(const char *, void *, void *), void *extra);

bool abb_iter_post_recur(nodo_abb_t* nodo,bool visitar(const char *, void *, void *), void *extra);

bool guardar_clave_valor(const char* clave,void* valor,void* array);
...
abb_item_t* abb_obtener_items(const abb_t* arbol){
	size_t cantidad = abb_cantidad((abb_t*)arbol);
	abb_item_t* array_items = malloc(sizeof(abb_item_t)*cantidad);
	if(!array_items) return NULL;
	nodo_abb_item_t* nodo_aux = malloc(sizeof(nodo_abb_item_t));
	if(!nodo_aux){
		free(array_items);
		return NULL;
	}
	nodo_aux->pos = 0;
	nodo_aux->array = array_items;
	abb_in_order((abb_t*)arbol,guardar_clave_valor,nodo_aux);
	free(nodo_aux);
	return array_items;
}

bool guardar_clave_valor(const char* clave,void* valor,void* extra){
	size_t pos = ((nodo_abb_item_t*)extra)->pos;
	abb_item_t* arreglo = ((nodo_abb_item_t*)extra)->array;
	arreglo[pos].clave = clave;
	arreglo[pos].valor = valor;
	((nodo_abb_item_t*)extra)->pos++;
	return true;
}
...
bool abb_iter_post_avanzar(abb_iter_post_t *iter){
	...
	nodo_abb_t* actual = pila_desapilar(iter->pila_nodos);
	if(pila_esta_vacia(iter->pila_nodos))return true;
	nodo_abb_t* padre = pila_ver_tope(iter->pila_nodos);
	if(actual==padre->izq){
		if(padre->der){
			actual = padre->der;
			pila_apilar(iter->pila_nodos,actual);
...
void abb_in_order(abb_t *arbol, bool visitar(const char *, void *, void *), void *extra){
	abb_iter_in_recur(arbol->raiz,visitar,extra);
	return;
}

/* *****************************************************************
 *                    ITERADOR INTERNO POSTORDER
 * *****************************************************************/

void abb_post_order(abb_t *arbol, bool visitar(const char *, void *, void *), void *extra){
	abb_iter_post_recur(arbol->raiz,visitar,extra);
	return;
}
...
bool abb_iter_in_recur(nodo_abb_t* nodo,bool visitar(const char *, void *, void *), void *extra){
	...
	bool estado_izq = abb_iter_in_recur(actual->izq,visitar,extra);
	if(!estado_izq) return false;
	bool estado_raiz = visitar(actual->clave,actual->dato,extra);
	if(!estado_raiz) return false;
	return abb_iter_in_recur(actual->der,visitar,extra);
}

bool abb_iter_post_recur(nodo_abb_t* nodo,bool visitar(const char *, void *, void *), void *extra){
	nodo_abb_t* actual = nodo;
	if(!actual) return true;
	bool estado_izq = abb_iter_post_recur(actual->izq,visitar,extra);
	if(!estado_izq) return false;
	bool estado_der = abb_iter_post_recur(actual->der,visitar,extra);
	if(!estado_der) return false;
	return visitar(actual->clave,actual->dato,extra);
}
